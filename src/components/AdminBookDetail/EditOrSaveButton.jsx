...
const EditOrSaveButton = ({
  ...
  function checkMissingFields(bookFields) {
    ...
    const missingFields = requiredKeys.filter((key) => !bookFields[key]);
  ...
  const handleEditOrSave = async (event) => {
    ...
    if (isEditing) {
      // 수정 updateBook.api
      try {
        checkMissingFields(selectedBook);
